...
class DespesasFragment() : Fragment(), LoaderManager.LoaderCallbacks<Cursor> {
    ...
    override fun onCreateView(
        ...
        val despesasViewModel =
            ViewModelProvider(this).get(DespesasViewModel::class.java)

        _binding = FragmentDespesasBinding.inflate(inflater, container, false)
        val root: View = binding.root

        //val textView0: TextView = binding.textDespesas
        despesasViewModel.text.observe(viewLifecycleOwner) {
            //textView0.text = it
        }
        return root
    }
